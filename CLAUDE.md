...
1. **`scripts/poll_espn.py` is blocked by an AWS WAF bot challenge on ESPN's API - needs a real-browser-driven rewrite before it can work (discovered 2026-08-28, first draft night).** A live auth check against `mDraftDetail` with valid, correctly-formatted cookies (`espn_s2` + `SWID`, confirmed correct by inspecting them directly) still returned `202` with an empty body and a `x-amzn-waf-action: challenge` response header - AWS WAF's bot-challenge response, not an ESPN auth rejection. A plain `requests.get()` (even with cookies + a browser `User-Agent`) can't pass this; the challenge requires executing JS, which only happens in a real browser. This is a materially bigger build than the current script - most likely a headless-browser approach (Selenium/Playwright) that loads the page for real and either scrapes the rendered draft state or lets the browser's own solved challenge/cookies ride along on the request. **Decision for the 2026-08-28 draft: go fully manual on all 3 ESPN leagues (1, 3, 4) for tonight** - the manual DRAFTED button is unaffected by any of this (it only talks to Firebase, never ESPN) and is fully tested. Revisit the poller's architecture before the next draft (2026-08-30), without draft-night time pressure.
2. **Grab fresh ESPN auth cookies (`espn_s2`, `SWID`) into a local `.env` before each draft** - still correct groundwork regardless of the WAF blocker above (needed once the poller is rearchitected). Save as `ESPN_S2=...` / `SWID=...` (include SWID's literal `{braces}`) in a `.env` at the repo root; see `.env.example`.

**Done:**
- Firebase project creation (console setup) — `docs/firebase-config.js` holds real (non-placeholder) config; a manual `curl PUT`/`GET`/`DELETE` round-trip against `/leagues/league_1/drafted/{scratch_id}` confirmed live writes work end-to-end.
- `scripts/poll_espn.py` — written and self-tested (id-map generation, the Firebase write path, its own error paths for missing `.env`/id map, and its auth pre-check correctly detecting an ESPN-side rejection). **Currently non-functional against the real ESPN API** due to the AWS WAF blocker above (item 1) - not yet usable for live polling until that's resolved. Loads cookies from `.env` (hand-rolled parser, no new dependency), filters `leagues.yaml` to the 3 live ESPN leagues, polls `mDraftDetail` every 12s, maps ESPN's numeric `playerId` through `data/espn/espn_id_map.json`, and `PUT`s straight to the same Firebase REST path the manual DRAFTED button uses. K/DST picks (positions the board doesn't rank) are recognized and skipped silently rather than logged as mapping misses.
- `scripts/parsers/espn.py` also emits each player's ESPN numeric id (`espn_id` field) and `load_non_board_espn_ids()` (K/DST ids the parser otherwise drops), both consumed by `scripts/build.py` to generate `data/espn/espn_id_map.json` as `{"player_id_map": {...}, "non_board_espn_ids": [...]}` on every build — only as fresh as the last `build.py` run.
...
- Firebase realtime sync — drafted state at `/leagues/{league_id}/drafted/{player_id}` in Realtime Database, keyed by `player_id`, scoped per league; write-only button + `onValue` listener as single source of truth; listener re-subscribed on league switch; hard replacement for localStorage with a live sync-status pill; SDK lazy-imported so a CDN/config failure degrades to an unsynced-but-working board. `firebase-rules.json` (repo root) holds the structure-scoped security rules. Verified in-browser and via a manual curl round-trip against the live database (see "Firebase project creation" above) — both the frontend wiring and the underlying REST path are confirmed working end-to-end.
