...
1. **Grab fresh ESPN auth cookies (`espn_s2`, `SWID`) into a local `.env` before each draft** — the poller code is done and self-checks these on startup (see below), but the cookie *values* aren't persisted anywhere and expire/rotate, so this is a per-draft-day step, not a one-time setup. Grab them close to the actual draft (same DevTools "Copy as cURL" method as the projections refresh) and save as `ESPN_S2=...` / `SWID=...` (include SWID's literal `{braces}`) in a `.env` at the repo root.
...
- ESPN live draft polling (3 ESPN leagues, `platform: espn` + `draft_mode: live`) with manual fallback — `scripts/poll_espn.py`: loads cookies from `.env` (hand-rolled parser, no new dependency), filters `leagues.yaml` to the 3 live ESPN leagues, runs a one-GET-per-league auth check before the polling loop (turns expired cookies into a loud, immediate failure instead of looking like "the draft hasn't started"), then polls `mDraftDetail` every 12s, maps ESPN's numeric `playerId` through `data/espn/espn_id_map.json`, and `PUT`s straight to the same Firebase REST path the manual DRAFTED button uses. K/DST picks (positions the board doesn't rank) are recognized and skipped silently rather than logged as mapping misses. Verified: id-map generation (507 mapped / 0 unmapped, Christian McCaffrey spot-checked), the Firebase write path (manual curl round-trip), and the script's own error paths (missing `.env`, missing id map). **Not yet verified: a real GET against ESPN's API with live cookies** — do this once `.env` is populated, ahead of draft day (see item 1).
