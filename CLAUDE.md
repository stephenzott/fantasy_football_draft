...
### 6. Frontend (Static HTML/CSS/JS) — built, multi-league
Files live in `docs/`:
- `index.html` — page structure (includes the league `<select>` in the header)
...
- "DRAFTED" button per player — dims and strikes through the row. Drafted state is keyed by `player_id` and stored **per league** in `localStorage` under `draftboard_v1_drafted_{league_id}` (never a flat/global list). The storage key is resolved from the *current* league at every read/write, so in-tab league switching can't bleed state. (Firebase will later replace localStorage — same per-league / `player_id` keying.)
...
1. Real-time sync backend decision + implementation (replaces `localStorage`) — same per-league / `player_id` keying the frontend already uses, so this is a storage swap, not a re-model
2. ESPN live draft polling (3 ESPN leagues) with manual fallback
3. ESPN auth cookie handling (`espn_s2`, `SWID`) for private leagues — note this is a separate concern from the one-time projections pull above: draft-day polling needs a persistent local `.env`, the projections pull was a one-time transient fetch

**Done:**
- Multi-league frontend — league `<select>` populated from `docs/leagues.json`, active league via `?league=` (bad/missing param falls back to first league + rewrites URL), drafted state scoped per league in `localStorage` under `draftboard_v1_drafted_{league_id}` and keyed by `player_id`, per-league tab titles. Combined `players.json` fetched once and re-indexed in memory on switch. Verified in-browser: two-tab isolation and in-tab switching keep drafted sets separate; same WR shows the expected full-PPR vs no-PPR point gap per league.
